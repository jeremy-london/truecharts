    ...
    try:
        ...
        try:
            ...
            if len(numbers) >= 2:
                ...
                logger.warning(
                    f"Using fallback version parsing for '{version_string}' -> '{fallback_version}'"
                )
                return (parsed_version, version_string)
        except:
            pass
...
def cleanup_version_directories(
    app_path: Path, keep_versions: List[str], dry_run: bool = False
) -> List[str]:
...
def cleanup_json_file(
    ...
    try:
        ...
        if not isinstance(data, dict):
            logger.warning(
                f"app_versions.json in {app_path} is not a dictionary. Skipping JSON cleanup."
            )
            return False, []
        ...
        if dry_run:
            logger.info(
                f"[DRY RUN] Would remove from app_versions.json: {removed_versions}"
            )
...
def cleanup_application(
    app_path: Path, keep_count: int = 3, dry_run: bool = False, update_json: bool = True
) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Found versions in {search_dir.relative_to(app_path.parent) if search_dir != app_path else app_path.name}: {sorted(available_versions)}"
        )
        ...
        if update_json:
            json_success, json_removed = cleanup_json_file(
                app_path, keep_versions, dry_run
            )
...
def cleanup_repository(
    ...
    logger.info(
        f"{'Would keep' if dry_run else 'Keeping'} {keep_count} versions per application"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-json", action="store_true", help="Skip updating app_versions.json files"
    )
